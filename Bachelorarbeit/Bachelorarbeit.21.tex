...
Fakultät Humanwissenschaften\hspace{2.5cm} & \tabularnewline
 & \tabularnewline
 & \tabularnewline
 & \tabularnewline
\end{tabular}
...
eingereicht von:\hspace{1cm} & Linus Edelkott \tabularnewline
 ...
 & \tabularnewline
Betreuer: & MS Julius Schöning\tabularnewline
 & Universität Osnabrück\tabularnewline
...
\section{Clustering}
% TODO: insert space before every \cite{
...
way that samples within a cluster are more similar to each other than samples
...
of samples within a cluster, whereas the latter is the distance
of one cluster to another one \cite{Ray1999}.

The similarity measurement reflects a priori knowledge, for
...
% TODO: clarify what ad hoc means?
the clustering process is usually ``ad hoc``; the underlying clustering
process defines the relevant information itself. This makes clustering
very generalizable \cite{VladimirCherkassky2007}.

Often clustering algorithms create so-called ``prototypes``. A prototype is a data
...
reduced output.
Thus the main characteristics of the data are preserved while
the amount of data is reduced.

Clusters can either be described hierarchically (i.e. described in
tree structures) or they can be purely partitional. The latter 
...
The second one is called ``soft clustering`` and 
...
sample to the closest cluster center and update the centroid by shifting it
to the mean of all cluster members for each attribute. This step has
% should probably mention convergence to local minimum here
to be repeated until the algorithm converges. Due to bad initialization points
...
\begin{algorithm}[H]
% here you should have used a \label or something so that you don't have to write algo 2.1
...
% the crosses still seem wrong, isn't there one cross for each cluster???
% why are there triangles in the first image and other stuff in the right image???
\begin{figure}[H]
...
describes the distance of a point to its centroid \cite{ChrisSolomon2011_km}. It can be shown
...
but most of the time the Euclidean distance is used. \\
% where did you get this? use something like https://en.wikipedia.org/wiki/Euclidean_distance
\[
\text{Euclidean distance: }\frac{1}{I}\cdot\forall i\left(\sqrt{x_{i}-c_{i}}\right)^{2}
\]

An advange of k-means clustering is its low complexity, but it also has
some weaknesses. K-means is sensitive to outliers, which highly influence
...
2.2. Finally it has the requirement of needing an accurate estimation of the expected number
...
with content based retrieval \cite{Lucchese_colorimage}. Hereby images
are organised/searched according to their content. 

Clustering algorithms share the same problem as region based segmentation algorithms
...
to choosing the number of clusters beforehand. If a good number of clusters
...
Although k-means clustering is relatively easy to implement on
a relatively wide field, it has some drawbacks. The final results of
...
from all points to their cluster centers. This means the distances from
each point to its cluster center provides the information whether
...
we want to minimize this measurement. The inter-cluster
...
In an optimal clustering we want to have compact clusters,
which are as distant from each other as possible. Thus the validity measurement is
defined as the ratio of the intra- to the inter-measure.
...
in Algorithm 2.2. It starts with initialising one cluster containing each data point
with their average as the cluster center
(line 1). In a loop, the variance for each cluster is
calculated 
(line 3) and the cluster with the largest value is split (line 4).
Due to the minimization of intra-cluster distances in k-means
% TODO: don't understand the following
...
k-means clustering is executed and the validity is calculated according
to the newly calculated cluster centers (line 5). This processes is
repeated until the parameterized maximum number of clusters $k_{\text{max}}$
...
With $z_{i}^{'}$ and $z_{i}^{''}$ being the two new cluster centers and
$z_{i}$ being the the old cluster centers with index $1,2,3$ for
each attribute. 
% TODO: and a_i?? explain the next equation.
...
to be selected more often. In synthetic images, with an objectively perfect
cluster count the calculated minimal validity will be the optimal
cluster count.
% The next sentence makes no sense
In natural images the best number of clusters is usually
...
It is important to notice that this algorithm is supposed to be used for image
...
of brightness values. The RGB color space is very popular.
It is based on the tristimulus theory of color in the human
visual system. Its name describes the 3 color channels used: red,
...
In 24-bit images, each channel is usually scaled to 0\textendash 255
...
non-linearity. Moving in a certain direction inside the cube might not produce
...
% the thing you're showing is not a hexagon, but a cone...
be represented in a 3d-hexagon, where the central vertical axis represents
...
changing lighting conditions \cite{ChrisSolomon2011}.
...
distances among colors perceived by the human eye are not mirrored by
similar distances between the points representing those colors in
such spaces \cite{Lucchese_colorimage}.
% and now I would expect a proposal of what color space to use or why there isn't a suitable one
...
processing them frame by frame. 
The resulting processing time, however, would prove too high for most practical uses
if the algorithm is
...
Hence, the first aim of this bachelor thesis is to find a way to
adapt the identification of the optimal cluster number from images
to video sequences. 

% don't understand 'investigation'
Without further investigation the resulting clusters have only a limited
use and can only represent the knowledge gathered in a
% include where?
single frame. In order to include the information of the whole video
it is necessary to compare the clusters of each frame to those of the neighbouring frames.
Clusters occuring in several frames should be recognized to enable tracking them.

Thus, the second aim of this bachelor thesis is to implement a similarity
measure for clusters to merge them over several frames. In order
...
The algorithm by Siddheswar Ray and Rose H. Turi can be transferred to video sequences
in a straightforward manner \cite{Ray1999}.
...
the calculation time while maintaining the same results that algorithm
2.2 would produce if applied to each frame individually.

Often the optimal cluster count will be unchanged from one frame to
the subsequent one. The resulting clusters will only vary slightly,
either due to a movement that occurred or due to changes in the lighting
conditions. It therefore seems reasonable to look for an effective way
to include the knowledge gained from previous frames. One option
...
% unclear sentence
in comparison to a higher/lower cluster count. This means that if
...
Unfortunately there are two major challenges for this conclusion. Even
...
the global minimum while for real images it would be the smallest
validity after the first local maximum. Neither for synthetic nor
Whether the current minimum is optimal requires the calculation of the validity at each k in both synthetic and
natural images.
...
% could be clearer
it is the minimal value after the first local maximum. But if the previous
...
used number of clusters in the previous one, the old cluster centers might
...
first with the mean of all pixels as the cluster center.
The cluster with the highest variance is then repeatedly split into two parts. Thus
...
Despite these problems, using the local minimum assumption
seems to be a good indicator to either keep the old cluster centers
as initialization points or to start over. The stated problems are
expected to occure rarely, due to the mostly small changes between
framres. The impact of identifying
the wrong cluster count in a video sequence will be further discussed
% you need to use labels for linking instead of hardcoding numbers...
in the evaluation of this algorithm in 4.2.

% a heading like 'explanation of the adaptive ... algorithm' would be good
To initialize Algorithm 3.1, the known Algorithm 2.2 will
...
in order to check whether the old cluster centers still constitute
a local minimum in the validity chart on the next frame. First the
...
5). To check whether the current validity constitutes a local minimum, the
...
clusters previously belonged together. Instead
of getting the validity for k-1 the corresponding cluster centers
...
cluster centers, because they might be required later. After running k-means
clustering and calculating the variance it can finally be checked
whether the validity for k constitutes a minimum (line 12).
...
and Algorithm 2.2 will be repeated on the current frame
(line 15).
...
This algorithm will speed up computational time with a loss in accuracy. In
some special cases it will provide non optimal solutions, which will
...
about the whole video sequence. The basic k-means color segmentation algorithm
as described in 2.2 and used repeatedly in 2.3, would ultimately provide
...
could be seen as a color histogram of the whole video sequence.
One improvement is to not only include the colors, but
also the sizes of the clusters in the similarity measure. Thus clusters which share a color
with another cluster but are ultimately unconnected might
...
any information about shape and may contain disjoint areas. For
example in Figure 3.2 the optimal cluster count based on colors would be determined
...
feature space from a 3d-vector which includes the colors to
...
segmentation and optimal cluster count of 3 when including geometrical features.}
\end{figure}
...
detects the edges of the respective cluster, before the openCV
method ``findContours(...)'' tries to calculate the corresponding
...
several RGB values. The core of the circle and
everything in close proximity has only one color value, but closer
to the boundary the color changes in order to
achieve an effect of anti-aliasing (Figure 4.2).
In this specific case the circle consists
...
nature of the RGB and HSV color spaces result in different
distances between colors. Many of the 49 RGB colors will probably
share the same hue value, but can be further clustered according to
their saturation and brightness. Therefore, this results in different
clusters in comparison to the RGB color space and thereby to a rise
of the calculated optimal cluster count.

Given that the number of colors occurring in natural images are expected to be
a lot higher than 53, the performed clustering will never be on a comparably
fine level. The decrease of the intra cluster distance would lead
to a lower inter cluster distance and presumably the validity would
increase.
...
testing images (Figure 4.3), resulted in a correct
...
Until now the cluster count was only tested on synthetic images
with a known optimal cluster count. On real images this count can usually only be estimated.

The algorithm was tested on images of the Berkeley Segmentation
...
clusters successively until $k_{\max}$ is reached. Therefore, the images
...
count where the red shirt is assigned its own cluster. It seems as in
a smaller cluster count the inter cluster distance is bigger. In
...
be similarly good. The cluster count of 10 seen in d) might be preferable
due to the existence of a cluster solely for the snow shoes. The latter
impression probably arises out of our focus on the image center. 
However, the sky has been segmented into 3 sections, which might not be desirable.
...
Using the HSV instead of the RGB color space did not lead
to considerably better results. Strikingly the calculated optimal
cluster count was mostly higher in HSV (Figure
4.7). Otherwise the same findings as in the RGB color space were
made. Overall, the comparison did not favor either color space. The results
were subjectively examined on the same level. The determination of
which color space to use should depend on the specific task of the clustering
algorithm. 
For simplicity and compatibility with the original algorithm, the RGB color space
...
Applying the dynamical cluster count algorithm (Algorithm
3.1) on a simple synthetic video sequence revealed that the cluster
count incremented correctly when a new cluster appeared, but failed reduce k
when clusters were leaving the frame.
...
new appearance of a rectangle in cluster e) leads to an increased cluster
count. In cluster g) the green rectangle is gone and a
decreased cluster count should be the result. Unfortunately,
...
\frame {\includegraphics[width=0.67\textwidth]
...
\caption{\foreignlanguage{ngerman}{A simple video sequence in which areas are moving in and out of the video.}}
\end{figure}
...
disparity can still be compensated. For the transformation from k to
...
are solely based on the calculation in previous frames. Therefore
...
count on all synthetic simple video sequences similar
to Figure 4.8 as long as the conditions remained constant. The considered
cases included appearing and disappearing areas both individually and simultaneously. 
...
of the clusters and the super-clusters were calculated. In
...
than a specified $\delta_{dis}$ the cluster center belonged
to the super-center. 
...
algorithm developed by Ray and Rose H. Turi (Algorithm 2.2),
...
expected, as the adapted algorithm will not
change the cluster count if the validity currently constitutes a local
minimum, whereas the unandapted algorithm will always go with the global
minimum.
...
case the adapted algorithm produced a different result in only in 3 of the 19 frames.
...
what humans expect, because regions with similar colors are already
...
and reasonable results when clustering single images. 
...
Where $\alpha$ is the scaling factor. Usually the colors will
...
4.11 shows an example of this; the white area represents
the area not contained in the specific cluster. While the algorithm
...
to better results in the case of Figure 4.11, but the
rectangles were the first cluster in k=2 only by chance. In order to split the right
...
and the right cluster is assigned manually, the algorithm didn't generate reasonable
results. The major problem here is to find a scale of the geometric
features compared to the color features. In some images this scale could be
...
3d-vector including only the color can be retained and each
...
Here one major problem needs to be considered. The cluster(s) which
...
is assigned to its corresponding cluster. The assignment can
...
Both assignment methods have some flaws as illustrated in Figure 4.13. The
...
\end{figure}
On simpler images (e.g. Figure 4.1) both methods work
...
Testing the described algorithm by Siddheswar Ray and Rose H. Turi on images
verified the results reported by them, although some flaws need to
...
a local minimum in the next frame the basic k-means algorithm has to
...
use case could be evaluated in this thesis. However, if semantic
knowledge could actually be implemented in the clustering process,
the recognition of the serial identity of clusters in video sequences would drastically
