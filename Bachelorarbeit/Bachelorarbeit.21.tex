...
use and can only represent the knowledge gathered in asingle frame.
...
detects the edges of the respective cluster, before the openCV method
``findContours(...)'' tries to calculate the corresponding contours.
Contours can be seen as boundaries, and in contrast to edges they
form closed curvatures. If there are several contours nested within
each other as for example seen in Figure 3.3, only the outermost is
relevant and will represent a cluster. In the case of the image in
Figure 3.3, only the most outer one is relevant and will represent
